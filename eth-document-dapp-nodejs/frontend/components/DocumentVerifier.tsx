  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const savedVerifications = JSON.parse(
        localStorage.getItem('documentVerifications') || '[]'
      );
      savedVerifications.unshift(verificationRecord);
      // Mantener solo las últimas 50 verificaciones
      if (savedVerifications.length > 50) {
        savedVerifications.pop();
      }
      localStorage.setItem('documentVerifications', JSON.stringify(savedVerifications));
